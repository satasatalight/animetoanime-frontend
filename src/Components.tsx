...
export function EntryCard({entry} : {entry: Entry}){
    return (
        <div className="flex-col outline-white outline-2 outline-offset-2 bg-slate-900 rounded-lg overflow-hidden">
...
export function EntryListView({entry, hook} : {entry: Anime | Staff, hook: Function}){
    ...
    return (
        <button type="button" className="flex h-40 w-full overflow-hidden outline-white outline-2 outline-offset-2 bg-slate-900 hover:bg-slate-600 rounded-lg cursor-pointer" onClick={() => hook(entry)}>
