...
export default function({setPage} : {setPage : Function}){
    ...
    useEffect(() => { let a = async () => {
        let response = await fetch("http://localhost:8080/getDailyGame?date=2026-06-11");
        if(response != null){
    ...
    return (
        <>
            <h1 className='pt-3'>Anime to Anime</h1>
            ...
            <button type="button" onClick={() => {setPage(<Game start={dailyData.anime1} end={dailyData.anime2}/>)}} className='mx-auto rounded-md bg-teal-500 px-4 py-3 text-white hover:bg-teal-400 w-3xs text-center mt-5 cursor-pointer'>
                Start
            </button>
        </>
    );
